...
fn main() {
    ...
    let mut logger = Logger::new(app_key, "./target/tmp_log/");
    // 添加计时开始点
    let start = Instant::now();
    for _ in 0..1 * 10000 {
        let _ = logger.log(get_random_text(100).unwrap_or_default().as_str());
    }
    // 计算总耗时
...
fn get_random_text(len: usize) -> Option<String> {
    let text = get_text();
    if text.len() < len {
        return None;
    }

    // 找到一个随机起点，注意 UTF-8 字符边界
    let max_start = text.len() - len;
    let mut rng = rand::rng();
    for _ in 0..10 {
        let start = rng.random_range(0..=max_start);
        let end = start + len;

        // 截取时确保不会破坏 UTF-8 字符边界
